...
class Product extends Component {
	...
	showPaginationNumbers() {
		...
		if (this.state.pagination) {
			...
			return this.renderPagination(paginationNumbers);
		}
	}

	renderPagination(paginationNumbers) {
			...
			return (
				<nav aria-label="Page navigation">
					<ul className="pagination pagination-custom col-md-6 offset-md-3">
						...
						{paginationNumbers.map((element, index) => {
							return (
								<li
									className={
	...
	getNameCategoryByID = id => {
		for (let i = 0; i < this.props.category.length; i++) {
			if (id === this.props.category[i]._id)
										...
										(element, index) => {
											return (
												<tr>
													...
													<td>
														{element.sizeProducts?.sizeProduct?.map(
															item => (
																<p
																	key={
																		item
